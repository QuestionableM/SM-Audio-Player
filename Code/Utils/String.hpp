...
namespace String
{
	template<size_t v_buffer_sz = 0x256>
	static const char* FormatCStr(const char* format, ...)
	{
		va_list args;
		static char v_buffer[v_buffer_sz];

		va_start(args, format);
		vsprintf_s(v_buffer, format, args);
		va_end(args);
	...
	template<size_t v_buffer_sz = 0x256>
	static const char* FormatCStr(const char* format, va_list va_arg_list)
	{
		static char v_buffer[v_buffer_sz + 1];
		vsprintf_s(v_buffer, format, va_arg_list);
	...
	inline T ToLower(const T& str)
	{
		static_assert(std::is_same_v<T, std::string> ||
			std::is_same_v<T, std::wstring>, "ToLower can only be used with the following types: std::string, std::wstring, wchar_t, char");
	...
	*/
	...
	/*inline std::string BytesToHexString(const std::vector<unsigned char>& bytes, const int& maxLength, const int& lineLength)
	{
		...
		{
			sstream << std::setfill('0') << std::setw(sizeof(unsigned char) * 2) << std::hex << static_cast<int>(bytes[i]);
